...
public class PlayerShop : MonoBehaviour
{
    ...
    public void TestRay()
    {
        if (shopUI.gameObject.activeSelf == true)
        {
            ...
            pShoter.laserPoint.SetPosition(0, pShoter.laserStartPoint.position);

            RaycastHit rayHit;
            if (Physics.Raycast(pShoter.laserStartPoint.position, pShoter.laserStartPoint.forward, out rayHit, Mathf.Infinity, buttonMask))
            {
